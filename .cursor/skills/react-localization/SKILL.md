...
The source language is Finnish. Most of the current UI text is in Finnish, and it should be moved to the Finnish localization files. The Finnish translation files are named `fi.json` in each namespace folder.
...
```bash
bunx eslint "apps/app/src/**/*.{ts,tsx}" --config eslint.i18n.config.js
```
...
Location: `src/i18n/{{namespace}}/{{language}}.json`
...
Create new namespace files when no suitable namespace exists. All languages need to have the same namespace files, so add a dummy json file to all other languages than `fi` if a new namespace file is needed. The dummy files should only contain an empty object, no keys or values.

Ensure that the namespace files do not contain duplicated keys.

## Translation Hook

Import from `@aikoa/i18n/react`:

```tsx
import { useTranslation } from "@aikoa/i18n/react";
```
...
// Single namespace
const { t } = useTranslation("common");
t(($) => $.navigation.next); // The namespace is not included in the path.

// Multiple namespaces - first namespace is the default
const { t } = useTranslation(["feedback", "common"]);
t(($) => $.flow.title); // from feedback (first). The first namespace must NOT be in the path.
t(($) => $.common.navigation.next); // from common (prefixed). All namespaces apart from the first should be in the path.
```

It is very important to get the localization path correct. Otherwise, the translations will not be found. When a single namespace is used, the namespace is not included in the path. When multiple namespaces are used, the first namespace is not included in the path. The other namespaces must be included in the path.
...
```tsx
import { useTranslation, type TFunction } from "@aikoa/i18n/react";
...
export default defineConfig({
  locales: ["fi", "en", "sv"],
  extract: {
    input: ["apps/app/src/**/*.{ts,tsx}"],
    output: "src/i18n/{{namespace}}/{{language}}.json",
  },
});
```

## Regenerate localization types

After all localization tasks are complete, run the following command to regenerate the localization types:

```bash
bunx i18next-cli types
```

This will update the i18next types and remove type errors from the added translation keys.
