...
import { removeSkillFromMetadata, getInstalledSkills } from "../lib/metadata.js";
import { findWorkspaceRoot, getSkillOutputDir, detectAllAgents, agentConfigs, type AgentType } from "../lib/paths.js";
...
export async function removeCommand(skillName: string): Promise<void> {
  ...
  if (agentsWithSkill.length === 1) {
    ...
    p.log.info(`Found '${skillName}' installed for ${agentConfigs[targetAgent].name}`);
  } else {
  ...
  p.log.success(`Removed skill '${skillName}' from ${agentConfigs[targetAgent].name}`);
}
