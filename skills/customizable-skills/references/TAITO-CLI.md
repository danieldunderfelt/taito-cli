...
For skill authors: regenerate default files from `.taito/` templates using default values.
...
- **Customizable skills**: Detects `.taito/skill.config.toml`, prompts for values, renders templates, then outputs the customized files

## Metadata Tracking

Installed skills are tracked in `.taito-meta.json` in the skills directory:
